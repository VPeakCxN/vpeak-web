  ...
  try {
    ...
    if (studentResult.exists) {
      ...
      const createResponse = await fetch(`${baseUrl}/api/students/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid, name, regno, email, avatar }),
        credentials: 'include',
      });
    ...
    if (sessionInsertError) {
      ...
      return NextResponse.json(
        { error: 'Failed to create session', details: sessionInsertError.message },
        { status: 500 }
      );
    }

    // Set cookies server-side
    const userData: AuthCookieData = { uid, name, email, regno, avatar };
    cookieStore.set('session_id', session_id, cookieOptions);
    cookieStore.set('session_key', session_key, cookieOptions);
